...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    public Map<String, Object> generateOrder(OrderParam orderParam) {
        ...
        UmsMember currentMember = memberService.getCurrentMember();
        String orderLockKey = redissonLockService.getOrderLockKey(currentMember.getId());
        boolean lockAcquired = false;
        try {
            lockAcquired = redissonLockService.tryLock(orderLockKey, 3, 30, java.util.concurrent.TimeUnit.SECONDS);
            if (!lockAcquired) {
                Asserts.fail("下单请求过于频繁，请稍后重试");
            }
            ...
            if (lockAcquired) {
                redissonLockService.unlock(orderLockKey);
            }
        }
    }
