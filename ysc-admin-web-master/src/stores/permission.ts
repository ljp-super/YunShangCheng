...
export const usePermissionStore = defineStore('permission', () => {
  ...
  // 生成可访问的路由表
  const generateRoutes = (data: { menus: UmsMenu[]; username: string; roles: string[] }) => {
    const { menus, username, roles } = data
    let accessedRouters: RouteRecordExt[] = []
    
    const isNormalUser = username === 'user'
    
    if (isNormalUser) {
      accessedRouters = asyncRouterMap.filter(v => {
        return v.path.startsWith('/user/')
      })
    } else {
      accessedRouters = asyncRouterMap.filter(v => {
        if (v.path.startsWith('/user/')) return false
        if (v.path === '/home') return true
