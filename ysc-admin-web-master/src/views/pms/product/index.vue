    ...
    <div class="table-container">
      <el-table ref="productTable" :data="list" style="width: 100%" @selection-change="handleSelectionChange"
        ...
        <el-table-column label="商品图片" width="80" align="center">
          <template #default="scope"><img style="height: 60px" :src="scope.row.pic"></template>
        </el-table-column>
        <el-table-column label="商品信息" min-width="200">
          <template #default="scope">
            <p><strong>{{ scope.row.name }}</strong></p>
            <p style="color: #666; font-size: 12px;">品牌：{{ scope.row.brandName }} | 货号：{{ scope.row.productSn }}</p>
          </template>
        </el-table-column>
        <el-table-column label="价格" width="100" align="center">
          <template #default="scope"><strong style="color: #e6a23c;">￥{{ scope.row.price }}</strong></template>
        </el-table-column>
        ...
        <el-table-column label="状态" width="200" align="center">
          <template #default="scope">
            <div style="display: flex; flex-direction: column; gap: 4px;">
              <span :class="scope.row.publishStatus === 1 ? 'status-tag status-on' : 'status-tag status-off'">
                {{ scope.row.publishStatus === 1 ? '上架' : '下架' }}
              </span>
              <span :class="scope.row.newStatus === 1 ? 'status-tag status-new' : 'status-tag'">
                {{ scope.row.newStatus === 1 ? '新品' : '' }}
              </span>
              <span :class="scope.row.recommandStatus === 1 ? 'status-tag status-recommend' : 'status-tag'">
                {{ scope.row.recommandStatus === 1 ? '推荐' : '' }}
              </span>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="审核" width="90" align="center">
          <template #default="scope">
            <span :class="scope.row.verifyStatus === 1 ? 'status-tag status-pass' : 'status-tag status-wait'">
              {{ scope.row.verifyStatus === 1 ? '已通过' : '待审核' }}
            </span>
          </template>
        </el-table-column>
