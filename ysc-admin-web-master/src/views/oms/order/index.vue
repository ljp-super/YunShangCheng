...
const { isSuperAdmin, currentUsername } = usePermission()
...
const getList = async () => {
  ...
  try {
    ...
    let backendList = response.data.list || []
    // 数据权限控制：非超级管理员只能看到自己的订单
    if (!isSuperAdmin.value) {
      backendList = backendList.filter(o => o.memberUsername === currentUsername.value)
    }
    // 合并本地订单
    let localFiltered = localOrderStore.getFilteredOrders({
      orderSn: listQuery.value.orderSn,
      status: listQuery.value.status
    })
    // 数据权限控制：非超级管理员只能看到自己的本地订单
    if (!isSuperAdmin.value) {
      localFiltered = localFiltered.filter(o => o.memberUsername === currentUsername.value)
    }
...
const handleCloseOrder = (index: number, row: OmsOrder) => {
  ...
  closeOrderData.value.isLocal = (row as any)._isLocal
}

// 处理订单发货（本地订单直接更新状态）
const handleDeliveryOrder = (index: number, row: OmsOrder) => {
  if ((row as any)._isLocal) {
    localOrderStore.updateOrderStatus(row.id, 2)
    logStore.addLog('订单', '订单发货', `订单编号: ${row.orderSn}`)
...
const handleCloseOrderConfirm = async () => {
  ...
  if (closeOrderData.value.isLocal) {
    // 本地订单直接更新状态
    closeOrderData.value.orderIds.forEach(id => {
      localOrderStore.updateOrderStatus(id, 4)
    })
    logStore.addLog('订单', '关闭订单', `订单ID: ${closeOrderData.value.orderIds.join(',')}，备注: ${closeOrderData.value.content}`)
  } else {
    ...
    logStore.addLog('订单', '关闭订单', `订单ID: ${orderIds}，备注: ${closeOrderData.value.content}`)
  }
...
const deleteOrderFn = async (ids: number[]) => {
  ...
  // 判断是否有本地订单
  const localIds = ids.filter(id => localOrderStore.orders.some(o => o.id === id))
  const backendIds = ids.filter(id => !localOrderStore.orders.some(o => o.id === id))
  // 删除本地订单
  localIds.forEach(id => localOrderStore.deleteOrder(id))
  // 删除后端订单
  if (backendIds.length > 0) {
    await orderDeleteByIdsAPI({ ids: backendIds.join(',') })
  }
  logStore.addLog('订单', '删除订单', `订单ID: ${ids.join(',')}`)
  ElMessage({
    message: '删除成功！',
    type: 'success',
    duration: 1000
...
<template>
  <div class="app-container">
    <el-alert
      ...
      title="数据权限提示"
      type="info"
      description="您当前不是超级管理员，只能查看与自己相关的订单数据"
      show-icon
      :closable="false"
