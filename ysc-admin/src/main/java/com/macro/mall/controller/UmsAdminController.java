...
public class UmsAdminController {
    ...
    @Operation(summary = "登录以后返回token（Spring Security + JWT）")
    @RequestMapping(value = "/login", method = RequestMethod.POST)
    @ResponseBody
    public CommonResult login(@Validated @RequestBody UmsAdminLoginParam umsAdminLoginParam) {
        Map<String, Object> result = adminService.login(umsAdminLoginParam.getUsername(), umsAdminLoginParam.getPassword());
        if (!(Boolean) result.get("success")) {
            return CommonResult.validateFailed(result.get("message").toString());
        }
    ...
    public CommonResult getAdminInfo() {
        ...
        data.put("note", umsAdmin.getNote());
    ...
    @ResponseBody
    public CommonResult updateInfo(@RequestBody UmsAdmin admin) {
        ...
        updateAdmin.setNote(admin.getNote());
        updateAdmin.setPassword(currentAdmin.getPassword());
        int count = adminService.update(currentAdmin.getId(), updateAdmin);
        if (count > 0) {
            return CommonResult.success(count);
        }
        return CommonResult.failed();
